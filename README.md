...
Both formats begin with a thumbnail containing the four strongest selected matchups, shown as paired team logos over a separate stock field photograph. Its only printed text is the week and **College Football Predictions**. Selecting fewer than four games produces fewer pairs. The deck then shows one slide per game, from the weakest average power ranking to the strongest. Both ordering and thumbnail selection use your current national power rankings, with unranked opponents placed below the ranked teams for sorting.

**Shorts** uses its own portrait layout with vertically arranged thumbnail matchups, large team logos, and separate winner and spread result rows. **Edit games** returns to the format selector; switching between YouTube and Shorts keeps your selected games, lines, and picks. Fullscreen fits the chosen aspect ratio, and PNG/SVG downloads use the exact selected dimensions, also included in their filenames.
